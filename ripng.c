...
do_opt(const char *opt, const char *arg, sendip_data *pack) {
	...
	switch (opt[1]) {
	case 'v': /* version */
		rippack->version = opt2inth(arg, NULL, 1);
		pack->modified |= RIPNG_MOD_VERSION;
		break;
	case 'c': /* command */
		rippack->command = opt2inth(arg, NULL, 1);
	...
	case 'e': /* rip entry */
		...
		NEXT_FIELD
		ripopt->tag = len == 0 ? 0 : opt2intn(p, NULL, 2);
		NEXT_FIELD
		ripopt->len = len == 0 ? 128 : opt2inth(p, NULL, 1);
		NEXT_FIELD
		ripopt->metric = len == 0 ? 16 : opt2inth(p, NULL, 1);
		break;
