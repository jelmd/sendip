...
{
	wesp_header *wesp = (wesp_header *) pack->data;
	/* Sanity check - if the user is doing something funky and the next header
	   is not esp, don't try to interpret it as such. */
	...
	if (!(pack->modified & WESP_MOD_HDRLEN)) {
		...
		/* hdrlen is the length from the start of WESP header to the
		   start of the post-IV payload.  */
			wesp->hdrlen =
				pack->alloc_len + sizeof(struct ip_esp_hdr) + priv->ivlen;
		}
	}
